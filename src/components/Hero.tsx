...
const Hero = () => {
  ...
  const handleCleanFiles = async () => {
    ...
    if (!window.electron?.ipcRenderer && hasLargeFiles) {
      toast({
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.electron && window.electron.ipcRenderer) {
        ...
        for (const file of files) {
          try {
            ...
            if (isImage) {
              ...
              const result = await window.electron.ipcRenderer.invoke('clean-image', tempPath);
              if (result.success) {
              ...
              const result = await window.electron.ipcRenderer.invoke('clean-video', tempPath, outputPath);
              if (result.success) {
                cleaned.push(file.name);
              }
            }
        ...
        try {
          ...
          if (window.isElectronApp || window.electron) {
