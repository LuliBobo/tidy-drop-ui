...
import { registerUser, verifyUser, logoutUser, isUserLoggedIn, getCurrentUsername, getAllUsers, updateUser, deleteUser, isCurrentUserAdmin, getCurrentUserRole, initiatePasswordReset, completePasswordReset, exportUserData, importUserData } from '../backend/auth.js';
...
export async function blobUrlToTempFile(blobUrl, fileName) {
...
ipcMain.handle('clean-image', async (_, filePath) => {
    return await cleanImage(filePath);
});
ipcMain.handle('clean-video', async (_, input, output) => {
    return await cleanVideo(input, output);
});
ipcMain.handle('create-zip', async (_, files) => {
...
ipcMain.handle('send-feedback', async (_, feedback) => {
    try {
        ...
        try {
            ...
            const feedbackFile = path.join(feedbacksDir, `feedback-${new Date().toISOString().replace(/:/g, '-')}.json`);
...
ipcMain.handle('update-user', async (_, username, updatedData) => {
    // Check if the current user is an admin
    if (!isCurrentUserAdmin()) {
        return { success: false, error: 'Unauthorized access' };
    }
...
ipcMain.handle('initiate-password-reset', async (_, username) => {
    return initiatePasswordReset(username);
});
ipcMain.handle('complete-password-reset', async (_, username, code, newPassword) => {
    return completePasswordReset(username, code, newPassword);
});
// IPC handlers for data import/export
ipcMain.handle('export-user-data', async (_, exportPath) => {
    return exportUserData(exportPath);
});
ipcMain.handle('import-user-data', async (_, importPath, mode) => {
