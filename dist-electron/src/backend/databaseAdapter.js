...
(function (PlatformMode) {
    PlatformMode["ELECTRON"] = "electron";
    PlatformMode["WEB"] = "web";
...
export class DatabaseAdapter {
    mode;
    postgresAdapter = null; // Dynamic type for cross-platform compatibility
    isInitialized = false;
    ...
    // Current user cache
    currentUser = null;
    constructor(mode = PlatformMode.ELECTRON) {
        this.mode = mode;
     ...
     */
    async initialize(postgresConfig) {
        try {
     ...
     */
    async createBackup(operationType) {
        try {
            if (!this.isInitialized) {
                throw new Error('DatabaseAdapter not initialized');
            }
     ...
     */
    async addAuditLogEntry(action, username, details) {
        try {
            if (!this.isInitialized) {
                return false;
            }
     ...
     * Export user data
     */
    async exportUserData(exportPath) {
        try {
            if (!this.isInitialized) {
                throw new Error('DatabaseAdapter not initialized');
            }
     ...
     * Import user data
     */
    async importUserData(data, mode = 'merge') {
        try {
            if (!this.isInitialized) {
                throw new Error('DatabaseAdapter not initialized');
            }
            ...
            if (this.mode === PlatformMode.WEB && this.postgresAdapter) {
                ...
                if (mode === 'replace') {
                    ...
                    for (const importedUser of importData.users) {
                        const existingUserIndex = resultUsers.findIndex(u => u.username === importedUser.username);
